...
@template(async ({ state, gameStateObjects }: HostSetupScreenProps) => {
    ...
    function proceedToWaiting() {
        if (gameStateObjects.questions.length === 0)
            return failureSnackbarMessage('Error', 'Please import a quiz that contains questions.', 2000);
        state.val = 'waiting';
    }
    ...
    return (
        <section class="l-row flex">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <div>
                    <Separator text="or without an Account" />
					<div class="button-row">

                    <button class="create-quiz-button" type="button" onclick={() => redirect('/createQuiz')}>
                        Create Quiz
                    </button>
                    <QuizImport gameStateObjects={gameStateObjects} />
					</div>
                </div>
                <button type="button" class="button" onclick={proceedToWaiting}>
                	Proceed to Lobby
                </button>
            </div>
        </section>
